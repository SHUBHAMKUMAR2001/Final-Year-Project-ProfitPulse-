...
tab1, tab2, tab3 = st.tabs(["The Global Stock Exchanges", "Stock Index Dashboard", "Portfolio Simulation"])

with tab1:
	st.markdown('<h1 style="text-align: center;color:yellow;">ProfitPulse: Smart Financial Decisons</h1>', unsafe_allow_html=True)
...
with tab2:
	st.write('<span style="color:yellow;font-size:30px;font-weight:bold">Stock Indices Historical Data</span>',unsafe_allow_html=True)
	...
	def optimize_return(df_dayReturn, max_variance=1, n_indices=6, n_portfolios=5000, random_seed=99):
		...
		while num_valid_portfolios < n_portfolios:
			...
			while count < 1:
				...
				try:
					problem.solve(solver=cp.ECOS)  # Explicitly specify the solver as ECOS
...
#st.text("")
with tab3:
	st.write('<span style="color:yellow;font-size:30px;font-weight:bold">Portfolio Efficient Frontier Simulation',unsafe_allow_html=True)
